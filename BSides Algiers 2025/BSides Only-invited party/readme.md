...
# BSides Only-invited party — Writeup

## TL;DR
- Reentrancy in `withdrawGoldenTicket` lets us inflate the player's balance while returning locked ETH to `0`.
- `_transfer` sets `balances[from] = 0`, so transferring the boss's ticket wipes the boss balance.
- The RPC proxy blocks `eth_sendTransaction` but does **not** block `eth_sendTransactionSync`, which Anvil supports. That lets us send a tx as the unlocked boss account.
...
## Exploit Outline
1. Deploy a helper contract (`BalanceBooster`) that:
   - Buys a ticket.
...
Usage:
```
python solve.py --rpc <RPC_URL> --priv <PLAYER_PRIV> --setup <SETUP_ADDR>
```
